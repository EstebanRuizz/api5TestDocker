...
export class CityControllerHttpOmitId extends OmitType(UpdateActionDTOCity, [
  'Id',
] as const) {}
...
export class CityControllerHttp {
  ...
  public async patchById(
    @Param('Id') Id: string,
    @Body() body: CityControllerHttpOmitId,
  ): Promise<City> {
  ...
  public async Delete(@Param('id') id: string): Promise<City> {
    return await this.commandBus.execute(
      await this.paramIdDtoValidatorCqrs.validateIdDTO(DeleteActionDTOCity, id),
    );
  }
}
